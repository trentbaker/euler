...
object Problem11 : EulerProblem() {
    ...
    private fun getLinearInDirection(
        grid: List<List<BigInteger>>,
        start: Pair<Int, Int>,
        direction: Direction,
        length: Int
    ) =
        (0 until length).mapNotNull {
            grid.getOrNull(start.second + (direction.yDir * it))
    ...
    override fun exampleProblem(): String = buildString {
        append("What is the greatest product of two adjacent numbers in the same direction (up, down, left, right, or diagonally) in the 20×20 grid: ")
        val result = largestProductOfAdjacent(TWENTY_SQUARE_GRID, 2)
        append(result)
    }

    override fun realProblem(): String = buildString {
        append("What is the greatest product of four adjacent numbers in the same direction (up, down, left, right, or diagonally) in the 20×20 grid: ")
        val result = largestProductOfAdjacent(TWENTY_SQUARE_GRID, 4)
        append(result)
    }
}

fun main() {
    println(Problem11.solve())
}