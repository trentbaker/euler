...
import NAMES_FILE
import amicableNumbersBelow
import bigFibonacciSequence
import calculateNameScores
import lexicographicSequence
import over
import timed
...
fun main() {
    ...
    //         // IntRange(0, 28123).toList().main.cannotSumFromAbundant().sum() // took 88.247s
    //         4179871
    // )
    // println(
    ...
    //     bigFibonacciSequence().mapIndexed { index, it -> it to index }.filter { it.first.toString().length == 1000 }
    //         .first().second
    // )
    timed {
        println(
